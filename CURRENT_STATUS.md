...
| Date | Summary |
|---|---|
| 2026-06-13 | **Auth fixes — redirect host + cross-device confirmation:** (1) `/auth/callback` was building redirects from `request.url`, which behind Caddy is the container bind (`0.0.0.0:3000` per `HOSTNAME` in the Dockerfile) — so every email/magic-link redirect (success or failure) bounced to an unreachable `https://0.0.0.0:3000/...`. Fixed via new `lib/auth-url.ts#publicOrigin` (reads `x-forwarded-host`/`-proto`). (2) Email confirmation moved from the PKCE code flow (same-device only) to the token-hash flow: new `app/auth/confirm/route.ts` calls `verifyOtp({ type, token_hash })` server-side, so a teacher can register on their phone and confirm from any device. `/auth/callback` retained for OAuth. Added `safeNext()` open-redirect guard to both routes. **Requires a GoTrue email-template change** (point the link at `/auth/confirm?token_hash=…&type=…`) — see `docs/auth-email-templates.md`. Needs redeploy. `npx tsc --noEmit` clean. |
